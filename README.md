...
원본 CLI는 `~/ws/src/multi-protocol-gateway/docs/refine/refine.sh`. 이 도구는 그 셸을
TypeScript로 포팅·래핑해 웹 UI로 감싼다. 핵심 차이: **원본의 Step 5(.tex 자동 반영)는
사용자가 R항목별로 apply/skip/reject를 결정한 뒤 트리거**한다.
