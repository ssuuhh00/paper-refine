...
- **라운드 데이터**: `paper-refine/data/projects/<project_id>/rounds/<ts>_round_NNN/{1_review.md, 2_changes.md, 2_changes.json, 3_blind_test.md, 3_mapping.txt, 4_verdict.md, decisions.json}`
- **오답노트**: `paper-refine/data/projects/<project_id>/error_notes.md`
- **프로젝트 메타**: `${XDG_CONFIG_HOME:-~/.config}/paper-refine/projects.json`

`data/`는 `.gitignore`로 제외 — paper 리포에는 Apply된 `.tex` 변경분만 흘러들어가고, 결정 이력은 paper-refine 로컬에만 보존된다. 백업이 필요하면 `data/` 폴더를 통째로 복사.

레거시 마이그레이션: 기존 프로젝트 메타가 `<latex_root>/refine_output` 형태로 저장돼 있으면 `projectStore.list/get` 호출 시 새 경로로 자동 재기록 (파일 이동은 안 함, 메타만).
