...
원본 CLI(`~/ws/src/multi-protocol-gateway/docs/refine/refine.sh`)를 외부 도구로 포팅·래핑한다.
...
원본 refine.sh는 **Step 5에서 .tex 자동 반영 + 오답노트 추가**까지 수행하지만,
...
**(a) TypeScript로 포팅**. 원본 셸은 그대로 두고, `apps/server/src/pipeline/`에 재구현.

이유:
- 원본의 Step 5(자동 .tex 반영 + 오답노트 추가)를 우리는 사용자 Apply 시점으로 미뤄야 하므로 어차피 분기 필요
- Python heredoc 두 덩어리(블라인드 셔플 / 역매핑)는 TS로 옮겨도 합쳐서 ~150줄
...
- 페르소나 4종은 원본 `~/ws/src/multi-protocol-gateway/docs/refine/prompts/reviewer_{ieee,outsider,writing,structure}.md`. 첫 단계에선 paper-refine 안에 복사하지 않고 경로 참조로 충분 (프로젝트 모델에 prompts_dir 필드를 두고 default = 원본 경로)
